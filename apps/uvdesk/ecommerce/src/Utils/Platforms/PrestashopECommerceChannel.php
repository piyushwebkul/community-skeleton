...
use UVDesk\CommunityPackages\UVDesk\ECommerce\Utils\Api\Admin\StoreProperties\Prestashop;

class PrestashopECommerceChannel implements ECommerceChannelInterface
{
    const TEMPLATE = __DIR__ . "/../../../templates/configs/prestashop/store-template.php";
    ...
    {
        try {
            $response = Prestashop::get($this->getDomain(), $this->getClient());
            $this->id = $response['id'];
            $this->name = $response['name'];
            $this->timezone = $response['timezone']['value'];
            $this->ianaTimezone = $response['timezone']['value'];
            $this->currencyFormat = $response['currency']['symbol'];
            return true;
        } catch (\Exception $e) {
        }
    ...
    private function getOrderResponse($orderId)
    {
        ...
        curl_setopt($curlHandler, CURLOPT_URL, 'http://' . $this->getClient() . '@' . $this->getDomain() . '/api/orders/' . $orderId . '?output_format=JSON');
    ...
    private function getProductResponse($productId)
    {
        $curlHandler = curl_init();
        curl_setopt($curlHandler, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curlHandler, CURLOPT_URL, 'http://' . $this->getClient() . '@' . $this->getDomain() . '/api/products/' . $productId . '?output_format=JSON');
    ...
    private function getOrderDetails($orderId) {
        $curlHandler = curl_init('http://' . $this->getClient() . '@' . $this->getDomain() . '/api/order_details/' . $orderId . '?output_format=JSON');
        curl_setopt($curlHandler, CURLOPT_RETURNTRANSFER, 1);
        $curlResponse = curl_exec($curlHandler);
        curl_close($curlHandler);
    ...
    private function getOrderState($stateId) {
        $curlHandler = curl_init('http://' . $this->getClient() . '@' . $this->getDomain() . '/api/order_states/' . $stateId . '?output_format=JSON');
        curl_setopt($curlHandler, CURLOPT_RETURNTRANSFER, 1);
        $curlResponse = curl_exec($curlHandler);
        curl_close($curlHandler);
